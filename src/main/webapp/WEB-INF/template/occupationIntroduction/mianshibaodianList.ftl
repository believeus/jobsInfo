		...
		<div class="j_main_right">
			<div class="j_main_right_1">
				<p>
					...
					所在位置 > <a href="/jobIntroduce.jhtml" title="职业介绍">职业介绍</a>(<span>共${mianshibaodianService.total}条记录</span>)
				</p>
			</div>
			<div style="border:1px solid #e4e4e4;" class="j_main_right_2">
			<form id="listForm" action="mianshibaodianList.jhtml">
				<ul>
					[#list mianshibaodianService.content as msbd]
					<li>
						<a href="/mianshibaodian.jhtml?id=${msbd.id}">${msbd.title}</a><span>${msbd.editTime?number_to_datetime}</span>
					</li>
					[/#list]
				</ul>
				[@pagination pageNumber = mianshibaodianService.pageNumber totalPages = mianshibaodianService.totalPages]
