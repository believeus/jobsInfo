		...
		<div class="j_main_right">
			<div class="j_main_right_1">
				<p>
					...
					所在位置 > <a href="/jobIntroduce.jhtml" title="职业介绍">职业介绍</a>(<span>共${jianlizhinanService.total}条记录</span>)
				</p>
			</div>
			<div style="border:1px solid #e4e4e4;" class="j_main_right_2">
			<form id="listForm" action="jianlizhinanList.jhtml">
				<ul>
					[#list jianlizhinanService.content as jlzn]
					<li>
						<a href="/jianlizhinan.jhtml?id=${jlzn.id}">${jlzn.title}</a><span>${jlzn.editTime?number_to_datetime}</span>
					</li>
					[/#list]
				</ul>
				[@pagination pageNumber = jianlizhinanService.pageNumber totalPages = jianlizhinanService.totalPages]
