...
<title>${message("admin.order.view")} - Powered By e3dmall</title>
...
function onlineRefunds(id)
	{
		$.ajax({
			url:"onlineRefunds.jhtml",
			type:"POST",
			data:{sn:id},
			dateType:"json",
			cache:false,
			...
			{
				if(message=="error"){
						$.message({"type":"error","content":"操作错误"});
				}else if(message=="worn"){
						$.message({"type":"worn","content":"等待供应商确认退货"});
				}else{
						window.open(message,target='_blank');
				}
...
$().ready(function() {
					...
					$("#paymentForm").validate({
						rules: {
							amount: {
								...
								balance: [#if order.member!=null]${order.member.balance}[#else]0.0[/#if]
							}
						}
					});
				},
	...
	[#if !order.expired && order.orderStatus == "confirmed" && (order.paymentStatus == "paid" || order.paymentStatus == "partialPayment" || order.paymentStatus == "partialRefunds"|| order.paymentStatus == "applyRefunded")]
...
<body>
	...
	<table class="input tabContent">
		<tr>
			...
			<td>
				<input type="button" id="" onclick="onlineRefunds(${order.sn})" class="button" value="申请退款"[#if order.expired || order.orderStatus != "confirmed" || (order.paymentStatus != "paid" && order.paymentStatus != "partialPayment" && order.paymentStatus != "partialRefunds" && order.paymentStatus != "applyRefunded" )] disabled="disabled"[/#if] />
			</td>
			<td>
				<input type="button" id="paymentButton" class="button" value="确认支付"[#if order.expired  || (order.paymentStatus != "unpaid" && order.paymentStatus != "partialPayment")] disabled="disabled"[/#if] />
			</td>
			<td>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
						[#if order.paymentStatus=="paid"||order.paymentStatus=="refunded"]
							<span class="green">${message("Order.PaymentStatus." + order.paymentStatus)}</span>
						[/#if]
						[#if order.paymentStatus=="unpaid"]
							<span class="red">${message("Order.PaymentStatus." + order.paymentStatus)}</span>
						[/#if]
						[#if order.paymentStatus=="applyRefunded"]
	...
	<table class="input tabContent">
		...
		[#list order.refunds as refunds]
			<tr>
				...
				<td>
					<span title="${refunds.createDate}">${refunds.createDate}</span>
				</td>
			</tr>
		[/#list]
	</table>
