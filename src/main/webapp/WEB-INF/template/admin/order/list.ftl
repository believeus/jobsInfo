	...
	<form id="listForm" action="list.jhtml" method="get">
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="agreeRufunds">是否同意退款</a>
				</th>
			...
			[#list page.content as order]
				<tr>
					...
					<td>
						...
						<span class="green">${message("Order.OrderStatus." + order.orderStatus)}</span>
						[/#if]
						[#if order.orderStatus=="confirmed"]
						<span class="blue">${message("Order.OrderStatus." + order.orderStatus)}</span>
						[/#if]
						[#if order.orderStatus=="unconfirmed"||order.orderStatus=="cancelled"]
						<span class="red">${message("Order.OrderStatus." + order.orderStatus)}</span>
						[/#if]
					...
					<td>
						[#if order.shippingStatus=="unshipped"]
							<span class="red">${message("Order.ShippingStatus." + order.shippingStatus)}</span>
						[/#if]
						[#if order.shippingStatus=="shipped"||order.shippingStatus=="returned"]
							<span class="green">${message("Order.ShippingStatus." + order.shippingStatus)}</span>
						[/#if]
						[#if order.shippingStatus=="affirmGoods"]
							<span class="green">${message("Order.ShippingStatus." + order.shippingStatus)}</span>
						[/#if]
						[#if order.shippingStatus=="partialShipment"]
							<span class="green">${message("Order.ShippingStatus." + order.shippingStatus)}</span>
						[/#if]
					</td>
					<td>
					...
					<td>
						[#if order.agreeRufunds=="yes"]
							<span class="red">同意</span>
						[#elseif order.agreeRufunds=="no"]
							<span class="green">不同意</span>
						[/#if]
					</td>
					[@shiro.hasPermission name = "admin:print"]
						<td>
					...
					[/@shiro.hasPermission]
					<td>
						[#if order.agreeRufunds =="yes"|| order.paymentMethodName=="银行汇款"]
							<a href="view.jhtml?id=${order.id}">[${message("admin.common.view")}]</a>
						[#else]
							<span title="${message("admin.order.editNotAllowed")}">[${message("admin.common.view")}]</span>
						[/#if]	
					</td>
				</tr>
			[/#list]
